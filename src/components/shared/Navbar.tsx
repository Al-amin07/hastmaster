...
export default function Navbar() {
    ...
    return (
        <nav
            className={`z-50 top-0  left-0 w-full transition-all duration-300 ${scrolled  ? 'fixed bg-white shadow dark:bg-gray-800' : 'absolute bg-transparent text-white'
                }`}
        >
            <div className="container px-6 py-7 mx-auto md:flex md:justify-between md:items-center">
                ...
                {/* Mobile menu */}
                <div
                    className={`absolute inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out bg-white dark:bg-gray-800 md:mt-0 md:p-0 md:top-0 md:relative md:bg-transparent md:w-auto md:opacity-100 md:translate-x-0 md:flex md:items-center ${isOpen ? 'translate-x-0 opacity-100' : 'opacity-0 -translate-x-full md:opacity-100 md:translate-x-0'
                        }`}
                >
                    <div className="flex flex-col md:flex-row md:mx-6">
                        {navLinks.map((link) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                className={`my-2 ${ scrolled || pathname !== '/'? 'text-black': 'text-white'}  dark:text-gray-200 hover:text-blue-500 dark:hover:text-blue-400 md:mx-4 md:my-0`}
                            >
                                {link.label}
                            </Link>
